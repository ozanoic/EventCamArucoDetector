%% Multi-Window Sliding ArUco Detection for Event Camera Data (Optimized)
%  - Step through time in 1 ms ticks
%  - At each tick, try 10 different lookback windows (5ms..150ms)
%  - Run full detection pipeline on each window independently
%  - Log every detection (no deduplication, no early exit)
%  - Optimizations: binary search, accumarray, parfor (if available)
...
windowDurations_ms = [5, 10, 15, 20, 30, 50, 70, 100, 125, 150];
...
% Build dictionary (containers.Map not compatible with parfor, use arrays)
...
% Ensure sorted by time for binary search
...
%% ---- Preallocate output ----
% resultTable: [timestamp, anyDetected, id_win1, ..., id_win14]
...
if hasParallel
    % ====================================================================
    %  PARALLEL PATH: parfor over ticks with DataQueue for live printing
    % ====================================================================
    ...
    % DataQueue for real-time progress from workers
    dq = parallel.pool.DataQueue;
    parTicStart = tic;
    progressCount = containers.Map('count', 0);  % mutable counter
    ...
    parfor tick = 1:numTicks
        ...
        % Send progress ping via DataQueue
        send(dq, tick);
    end
    ...
    % ====================================================================
    %  SEQUENTIAL PATH: same logic, with progress printing
    % ====================================================================
    seqTicStart = tic;
    for tick = 1:numTicks
        ...
        for wi = 1:numWindows
            ...
            % Binary search for event range [tFrom, tNow]
            iStart = bsearchLeft(evT, tFrom);
            iEnd   = bsearchRight(evT, tNow);
            ...
            % Build event image using accumarray (vectorized)
            validMask = (wY >= 1) & (wY <= H) & (wX >= 1) & (wX <= W);
            wXv = wX(validMask);
            wYv = wY(validMask);
            if isempty(wXv), continue; end
...
saveData.tNow_us = resultTable(:, 1);
saveData.anyDetected = resultTable(:, 2);
for wi = 1:numWindows
    varName = sprintf('win_%dms', windowDurations_ms(wi));
    saveData.(varName) = resultTable(:, 2 + wi);
end
saveData.windowDurations_ms = windowDurations_ms;
saveData.detectionsPerWindow = detectionsPerWindow;

save('resultTable.mat', '-struct', 'saveData');
...
%% ---- Binary search: first index where evT >= tTarget ----
...
%% ---- Binary search: last index where evT <= tTarget ----
...
%% ---- orderCornersForUnwarp (local, parfor-compatible) ----
...
%% ---- unwarpQuad (local, parfor-compatible) ----
...
%% ---- decodeMarker (local, parfor-compatible, uses arrays not Map) ----
...
%% ---- Binary search in sorted dictionary codes ----
...
%% ---- buildDictionaryArrays (sorted arrays, parfor-compatible) ----
function [sortedCodes, sortedIDs] = buildDictionaryArrays()
    ...
    % Sort by code for binary search lookup
